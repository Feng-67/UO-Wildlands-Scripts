 ...
 *
 * Wave Spawner: Dreadhorn
 * Four waves centred on the spawner item within a 10-tile radius.
 * Place with [add WaveSpawnerDreadhorn, then GM double-click to start.
 * 1-hour cooldown after Dreadhorn dies.
 */
...
{
    public class WaveSpawnDreadHorn : Item
    {
        ...
        // ---------------------------------------------------------------
        [Constructable]
        public WaveSpawnDreadHorn() : base(0xED4)
        {
            Name    = "Wave Spawn: Dreadhorn";
            Movable = false;
            Visible = false;
            StartProximityTimer();
        }

        public WaveSpawnDreadHorn(Serial serial) : base(serial) { }
