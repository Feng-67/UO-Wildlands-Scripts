...
namespace Server.Multis
{
	public class PrisonerCamp : BaseCamp
	{
		...
		{
			AddItem(Gate = new IronGate(DoorFacing.EastCCW)
			{
		...
		public override void OnEnter(Mobile m)
		{
			...
			if (m.Player && Prisoner?.Deleted == false && Prisoner.CantWalk && (Gate?.Deleted != false || Gate.Locked))
			{
				int number;
