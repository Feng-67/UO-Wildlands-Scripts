	...
	public class XmlFindGump : Gump
	{
		public class XmlFindThread
		{
			...
			public void XmlFindThreadMain()
			{
				...
				var results = Search(m_SearchCriteria, out var status_str);

				var gump = new XmlFindGump(m_From, m_From.Location, m_From.Map, true, true, false,
				...
				_ = Timer.DelayCall(TimeSpan.Zero, new TimerStateCallback(GumpDisplayCallback), new object[] { m_From, gump, status_str });
			...
			public void GumpDisplayCallback(object state)
			{
				...
				var gump = (XmlFindGump)args[1];
				var status_str = (string)args[2];

				if (from != null && !from.Deleted)
				{
					_ = from.SendGump(gump);
					if (status_str != null)
					{
		...
		private static bool TestRegion(object o, string regionname)
		{
			...
			if (o is Item item)
			{
				...
				var r = Region.Regions.FirstOrDefault(reg => reg.Map == item.Map && !String.IsNullOrEmpty(reg.Name) && String.Equals(reg.Name, regionname, StringComparison.CurrentCultureIgnoreCase));
			...
			if (o is Mobile mob)
			{
				var r = Region.Regions.FirstOrDefault(reg => reg.Map == mob.Map && !String.IsNullOrEmpty(reg.Name) && String.Equals(reg.Name, regionname, StringComparison.CurrentCultureIgnoreCase));
		...
		private static void IgnoreManagedInternal(object i, ref ArrayList ignoreList)
		{
			...
			{
				_ = ignoreList.Add(deed.Commodity);
			}
			...
			if (i is BaseHouse house)
			{
				foreach (var relEntity in house.RelocatedEntities)
				{
					...
					{
						_ = ignoreList.Add(relEntity.Entity);
					}
				}
		...
		public static ArrayList Search(SearchCriteria criteria, out string status_str)
		{
			...
			if (itemarray != null)
			{
				foreach (Item i in itemarray)
				{
					...
					var hasregion = false;
					var hasmap = false;
					var hasspawnerr = false;
					var hasvalidhidden = false;
					...
					_ = newarray.Add(new SearchEntry(i));
				}
			}
		...
		public static void XmlFind_OnCommand(CommandEventArgs e)
		{
			...
			if (e.Arguments.Length > 1)
			{
				...
				try
				{
					range = Int32.Parse(e.Arguments[1]);
				}
				catch
				{
					dorange = false;
			...
			_ = e.Mobile.SendGump(new XmlFindGump(e.Mobile, e.Mobile.Location, e.Mobile.Map, typename, range, dorange, x, y));
		}
			...
			: base(X, Y)
		{
			...
			if (m_ShowExtension)
			{
				...
				for (var i = 0; i < MaxEntries; i++)
				{
					...
					var e = (SearchEntry)m_SearchList[index];
					...
					var ownstr = String.Empty;
					var texthue = 0;

					if (o is Item)
					{
						var item = (Item)e.Object;
						...
						if (o is Mobile)
					{
						var mob = (Mobile)e.Object;
						// change the color if it is in a container
						namestr = mob.Name;
						var str = mob.GetType().ToString();
						if (str != null)
						{
							var arglist = str.Split('.');
							typestr = arglist[arglist.Length - 1];
		...
		private void DoGoTo(int index)
		{
			...
			if (m_SearchList != null && index < m_SearchList.Count)
			{
				var o = ((SearchEntry)m_SearchList[index]).Object;
				if (o is Item item)
				{
					Point3D itemloc;
					if (item.Parent != null)
		...
		private void DoShowProps(int index)
		{
			...
			if (m_SearchList != null && index < m_SearchList.Count)
			{
				...
				if (o is Item x1)
				{
					...
					_ = m_From.SendGump(new PropertiesGump(m_From, x1));
				}
				else if (o is Mobile x2)
				{
					...
					_ = m_From.SendGump(new PropertiesGump(m_From, x2));
				}
			}
		}
		...
		private class ListTypeSorter : IComparer
		{
			...
			{
				var xstr = (e1 as SearchEntry)?.Object?.GetType().Name;
				var ystr = (e2 as SearchEntry)?.Object?.GetType().Name;
		...
		private class ListNameSorter : IComparer
		{
			...
			public int Compare(object e1, object e2)
			{
				var xstr = ((e1 as SearchEntry)?.Object as IEntity)?.Name;
				var ystr = ((e2 as SearchEntry)?.Object as IEntity)?.Name;
		...
		private class ListMapSorter : IComparer
		{
			...
			public int Compare(object e1, object e2)
			{
				var xstr = ((e1 as SearchEntry)?.Object as IEntity)?.Map.Name;
				var ystr = ((e2 as SearchEntry)?.Object as IEntity)?.Map.Name;
		...
		private class ListRangeSorter : IComparer
		{
			...
			public int Compare(object e1, object e2)
			{
				...
				var entity1 = (e1 as SearchEntry)?.Object as IEntity;
				var entity2 = (e2 as SearchEntry)?.Object as IEntity;
		...
		{
			_ = state.Mobile.SendGump(new XmlFindGump(m_From, StartingLoc, StartingMap, false, m_ShowExtension, Descendingsort, m_SearchCriteria, m_SearchList, Selected, DisplayFrom, SaveFilename,
		...
		private void ResetList()
		{
			...
			{
				var e = (SearchEntry)m_SearchList[i];
		...
		private void SaveList(Mobile from, string filename)
		{
			...
			var savelist = new List<XmlSpawner>();
			...
			_ = XmlSpawner.SaveSpawnList(from, savelist, dirname, false, true);
		}
		...
		private void ExecuteCommand(Mobile from, string command)
		{
			...
			if (!String.IsNullOrEmpty(command))
			{
				...
				if (args.Length > 1)
				{
					...
					var e = new CommandEventArgs(from, args[0], command, cargs);
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			var tr = info.GetTextEntry(105);        // range info
			...
			if (tr?.Text != null && tr.Text.Length > 0)
			{
				try { m_SearchCriteria.Searchrange = Int32.Parse(tr.Text); }
				catch (Exception e) { Diagnostics.ExceptionLogging.LogException(e); }
			}
			...
			switch (info.ButtonID)
			{
				...
				case 3: // Search
				{
					...
					var tobj = new XmlFindThread(state.Mobile, m_SearchCriteria, CommandString);
					var find = new Thread(tobj.XmlFindThreadMain)
					{
						Name = "XmlFind Thread"
					};
					find.Start();
				...
				{
					// do the search
					m_SearchList = Search(m_SearchCriteria, out var status_str);
					break;
				}
				...
				case 154: // Bring all selected objects to the current location
				{
					...
					_ = state.Mobile.SendGump(new XmlConfirmBringGump(m_SearchList));
					return;
				}
				...
				case 156: // Delete selected items
				{
					...
					_ = state.Mobile.SendGump(new XmlConfirmDeleteGump(m_SearchList));
					return;
				}
		...
		public class XmlConfirmBringGump : Gump
		{
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				...
				var myloc = state.Mobile.Location;
				var mymap = state.Mobile.Map;
				...
				switch (info.ButtonID)
				{
					default:
					{
						...
						{    // accept
							for (var i = 0; i < SearchList.Count; i++)
							{
								var e = (SearchEntry)SearchList[i];
