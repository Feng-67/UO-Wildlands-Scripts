...
{
	public class AllDyeTubsRecallRune : DyeTub
	{
...
//		private int TheHue = AllHuesInfo.Reds;
		...
		[Constructable]
		public AllDyeTubsRecallRune(  )
		{
			Name = "Dye Tub [Recall Rune]";
			Weight = 5.0;
			Hue = TheHue;
			DyedHue = TheHue;
			Charges = 5;
		...
		public void DoOut ( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				...
				from.Target = new AllDyeTubsRecallRuneTarget( this );
			}
			else
			{
		...
		public AllDyeTubsRecallRune( Serial serial ) : base( serial ){}
		...
		public class AllDyeTubsRecallRuneTarget : Target
		{
			private AllDyeTubsRecallRune m_Tub;

			public AllDyeTubsRecallRuneTarget( AllDyeTubsRecallRune dyetub ) : base( 12, false, TargetFlags.None )
			{
				m_Tub = dyetub;
			}
