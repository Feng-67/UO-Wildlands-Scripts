		...
		private static void ProcessSpawnerBookEntry(Mobile from, XmlSpawner spawner, int entryIndex, string entryContent)
		{
			...
			if (entryIndex < spawner.m_SpawnObjects.Count)
			{
				...
				if (so.TypeName != entryContent)
				{
					...
					CommandLogging.WriteLine(from, $"{from.AccessLevel} {CommandLogging.Format(from)} changed XmlSpawner {spawner.Serial} '{spawner.Name}' {loc} [{spawner.Map}] : {so.TypeName} to {entryContent}");
				}

				so.TypeName = entryContent;
			}
			else
			{
				// add a new spawn entry
				spawner.m_SpawnObjects.Add(new XmlSpawner.SpawnObject(from, spawner, entryContent, 1));
