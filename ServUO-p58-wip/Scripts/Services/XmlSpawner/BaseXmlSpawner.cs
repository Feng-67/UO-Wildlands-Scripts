...
namespace Server.Mobiles
{
	public class BaseXmlSpawner
	{
		...
		public static void RemoveKeyword(string name)
		{
			...
			_ = typemodKeywordHash.Remove(name);
					...
					if (!String.IsNullOrEmpty(m_Condition) && m_Spawner != null && m_Spawner.Running)
					{
						...
						if (TestItemProperty(m_Spawner, m_Spawner, m_Condition, out _))
						{
							...
							if (m_Goto >= 0 && m_Spawner != null && !m_Spawner.Deleted)
							{
								...
								// spawn the subgroup
								_ = m_Spawner.SpawnSubGroup(m_Goto, 0);
							}
		...
		public static string SetPropertyValue(XmlSpawner spawner, object o, string name, string value)
		{
			...
			if (arraystring.Length > 1)
			{
				...
				{
					_ = Int32.TryParse(arraystring[0], out index);
				}
			}
			...
			while (arglist.Length > 1)
			{
				...
				if (arglist.Length > 1)
				{
					...
					if (groupedarglist.Length > 1)
					{
						...
						var groupargs = ParseToMatchingParen(groupedarglist[1], '[', ']');
						...
						if (IsValuemodKeyword(value_keywordargs[0]))
						{
							...
							if (kw == valuemodKeyword.INC)
							{
								...
								if (value_keywordargs.Length > 1)
								{
									...
									{
										if (Int32.TryParse(value_keywordargs[1], out var min) && Int32.TryParse(value_keywordargs[2], out var max))
										{
									...
									var tmpvalue = GetPropertyValue(spawner, o, arglist[0], out var ptype);
									...
									else
									{
										...
										if (Double.TryParse(currentvalue, NumberStyles.Any, CultureInfo.InvariantCulture, out var d0) && Double.TryParse(incvalue, NumberStyles.Any, CultureInfo.InvariantCulture, out var d1))
										{
							...
							else if (kw == valuemodKeyword.PLAYERSINRANGE)
							{
								...
								if (value_keywordargs.Length > 1)
								{
									_ = Int32.TryParse(value_keywordargs[1], out range);
								}
								...
								if (refobject is Item)
								{
									...
									{
										if (p.Player && p.AccessLevel < AccessLevel.Counselor)
										{
											nplayers++;
										}
									}
		...
		public static bool TestMobProperty(XmlSpawner spawner, Mobile mobile, string testString, out string status_str)
		{
			...
			var testreturn = CheckPropertyString(spawner, mobile, testString, out status_str);
		...
		public static bool TestItemProperty(XmlSpawner spawner, Item ObjectPropertyItem, string testString, out string status_str)
		{
			...
			var testreturn = CheckPropertyString(spawner, ObjectPropertyItem, testString, out status_str);
		...
		public static string ParseForKeywords(XmlSpawner spawner, object o, string valstr, bool literal, out Type ptype)
		{
			...
			if (IsValueKeyword(pname))
			{
				...
				if (kw == valueKeyword.PLAYERSINRANGE && arglist.Length > 1)
				{
					...
					var nplayers = 0;
					// get the number of players in range
					_ = Int32.TryParse(arglist[1], out var range);
		...
		public static bool CheckPropertyString(XmlSpawner spawner, object o, string testString, out string status_str)
		{
			...
			if (arglist.Length < 2)
			{
				var returnval = CheckSingleProperty(spawner, o, testString, out status_str);
			...
			var first = CheckSingleProperty(spawner, o, arglist[0], out _);
			...
			var second = CheckPropertyString(spawner, o, arglist[1], out status_str);
		...
		public static bool CheckSingleProperty(XmlSpawner spawner, object o, string testString, out string status_str)
		{
			...
			var value1 = ParseForKeywords(spawner, o, arglist[0].Trim(), false, out var ptype1);
			...
			var value2 = ParseForKeywords(spawner, o, arglist[1].Trim(), false, out var ptype2);
			...
			if (ptype2 == typeof(TimeSpan) || ptype1 == typeof(TimeSpan))
			{
				if (hasequal)
				{
					if (TimeSpan.TryParse(value1, out var ts1) && TimeSpan.TryParse(value2, out var ts2))
					{
						if (ts1 == ts2)
						{
							return !invertreturn;
				...
				if (ptype2 == typeof(DateTime) || ptype1 == typeof(DateTime))
			{
				if (hasequal)
				{
					if (DateTime.TryParse(value1, out var dt1) && DateTime.TryParse(value2, out var dt2))
					{
						if (dt1 == dt2)
						{
							return !invertreturn;
		...
		public static XmlSpawner FindInRecentSpawnerSearchList(XmlSpawner spawner, string name)
		{
			...
			if (deletelist != null)
			{
				...
				{
					_ = spawner.RecentSpawnerSearchList.Remove(i);
				}
			}
		...
		public static Item FindInRecentItemSearchList(XmlSpawner spawner, string name, string typestr)
		{
			...
			if (deletelist != null)
			{
				foreach (var i in deletelist)
				{
					_ = spawner.RecentItemSearchList.Remove(i);
				}
			}
		...
		public static Mobile FindInRecentMobileSearchList(XmlSpawner spawner, string name, string typestr)
		{
			...
			if (deletelist != null)
			{
				foreach (var i in deletelist)
				{
					_ = spawner.RecentMobileSearchList.Remove(i);
				}
			}
		...
		public static string[] ParseString(string str, int nitems, string delimstr)
		{
			...
			str = str.Trim();
			var args = str.Split(delims, nitems);
		...
		public static string[] ParseCommaArgs(string str, int nitems)
		{
			...
			var args = str.Split(commadelim, nitems);
			return args;
		}
		...
		public static string[] ParseLiteralTerminator(string str)
		{
			...
			var args = str.Split(literalend, 2);
			return args;
		}
		...
		public static string[] ParseSemicolonArgs(string str, int nitems)
		{
			...
			var args = str.Split(semicolondelim, nitems);
			return args;
		}
			...
			List<XmlSpawner.SpawnPositionInfo> spawnpositioning, string propertyString, bool smartspawn, out string status_str)
		{
			...
			if (spawner != null)
			{
				...
				if (!spawner.Deleted)
				{
					...
					else
					{
						...
						var loc = spawner.GetSpawnPosition(requiresurface, theSpawn.PackRange, packcoord, spawnpositioning);
			...
			_ = ApplyObjectStringProperties(spawner, propertyString, item, trigmob, spawner, out status_str);
		}
				...
				switch (kw)
				{
					case typeKeyword.SET:
					{
						...
						if (keywordargs.Length > 1)
						{
							...
							_ = ApplyObjectStringProperties(spawner, substitutedtypeName, setitem, triggermob, invoker, out status_str);
						}
						else if (spawner != null)
						{
							_ = ApplyObjectStringProperties(spawner, substitutedtypeName, spawner.SetItem, triggermob, invoker, out status_str);
						}
					...
					case typeKeyword.SPAWN:
					{
						...
						if (targetspawner != null)
						{
							if (spawner != targetspawner)
							{
								...
								_ = targetspawner.SpawnSubGroup(subgroup, false, true, (byte)(loops + 1));
							}
							else
							{
								...
								_ = targetspawner.SpawnSubGroup(subgroup, (byte)(loops + 1));
							}
						}
						else
						{
					...
					case typeKeyword.COMMAND:
					{
						...
						if (arglist.Length > 0)
						{
							...
							if (CommandMobileName != null)
							{
								...
								if (dummy != null)
								{
									_ = CommandSystem.Handle(dummy, String.Format("{0}{1}", CommandSystem.Prefix, arglist[1]));
								}
							}
							else
								if (triggermob != null && !triggermob.Deleted)
							{
								_ = CommandSystem.Handle(triggermob, String.Format("{0}{1}", CommandSystem.Prefix, arglist[1]));
							}
						}
						else
						{
