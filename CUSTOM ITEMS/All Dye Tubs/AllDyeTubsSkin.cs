...
{
	public class AllDyeTubsSkin : DyeTub
	{
		...
		public void DoOut ( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				...
				from.Target = new AllDyeTubsSkinTarget( this );
			}
			else
			{
		...
		public AllDyeTubsSkin( Serial serial ) : base( serial ){}
		...
		public class AllDyeTubsSkinTarget : Target
		{
			private AllDyeTubsSkin m_Tub;

			public AllDyeTubsSkinTarget( AllDyeTubsSkin dyetub ) : base( 12, false, TargetFlags.None )
			{
				m_Tub = dyetub;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( ( targeted is PlayerMobile ) && ( targeted == from ) )
				{
					from.Hue = m_Tub.DyedHue;
