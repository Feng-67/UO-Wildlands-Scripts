...
namespace Server.Items
{
	...
	public class BaseBook : Item, ISecurable
	{
		...
		public BaseBook(int itemID)
			: this(itemID, DefaultPageCount, true)
		{
		}
		...
		public void BuildBookFromString(string content)
		{
			...
			const int cpl = 20; //characters per line
			int pos = 0, nextpos;
		...
		public string ContentAsStringNoBreaks
		{
			get
			{
		...
		public static void ContentChange(NetState state, PacketReader pvSrc)
		{
			...
			while (--pageCount >= 0)
			{
				int pageIndex = pvSrc.ReadUInt16() - 1;

				if (pageIndex < 0 || pageIndex >= book.PagesCount)
				{
					return;
				}
				...
				BookPageInfo page = book.Pages[pageIndex];
