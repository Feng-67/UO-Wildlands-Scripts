...
string broadcastMsg = String.Format("THE CITY OF {0} FELUCCA IS UNDER SIEGE BY {1}! TO ARMS, BRAVE SOULS - DEFEND THE REALM!", TownInvaded, TownMonsterType);
	...
	// This ensures the invasion disappears from the list gump 
    	// immediately when it ends or is stopped.
...
private static void GlobalSync()
{
    ...
    while (--index >= 0)
    {
        ...
        // Check if it's time to start AND if it hasn't already started
        if (obj._StartTime <= DateTime.UtcNow && !obj.IsRunning)
        {
            obj.OnStart();
            
            // This is the "Final Polish" - it updates the Master Gump status 
            // from Yellow (QUEUED) to Green (ACTIVE) automatically.
            InvasionControl.RefreshAllOpenGumps();
        }
    }
}
