...
namespace Server.Services.TownCryer
{
	public class TownCryerNewsGump : BaseTownCryerGump
	{
		...
		public override void AddGumpLayout()
		{
			...
			if (!System.String.IsNullOrEmpty(Entry?.InfoUrl))
			{
		...
		public override void OnResponse(RelayInfo info)
		{
			switch (info.ButtonID)
			{
				...
				case 2:
					...
					{
						var quest = QuestHelper.Construct(Entry.QuestType);
						...
						if (quest != null)
						{
							...
							{
								_ = User.CloseGump(typeof(MondainQuestGump));
								_ = User.SendGump(new MondainQuestGump(quest));
							}
							else
							{
								quest.Owner = null;
